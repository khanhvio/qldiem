...
export const HtmlHead: React.FC<Props> = ({ title, description, url, ogImage, favicon }) => {
  ...
  const defaultOGImage = `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/images/og-image.png`
  const defaultFavicon = `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/favicon.ico`
